...
export async function onRequestPost({ request, env }) {
  ...
  await env.DB.prepare(
    `INSERT INTO posts (id, title, body, image_url, created_at) VALUES (?, ?, ?, ?, ?)`
  ).bind(id, title, body, image_url || null, created_at).run();

  return Response.json({ id, title, body, image_url, created_at, reactions: [], comment_count: 0 });
}
